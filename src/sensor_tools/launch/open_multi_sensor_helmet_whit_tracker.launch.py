...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument('h_fisheye_port', default_value='70'),
        ...
        DeclareLaunchArgument('h_serial_port', default_value='/dev/ttyUSB70'),
        ...
        DeclareLaunchArgument('h_depth_camera_no', default_value='_230322272618'),
    ...
    locator_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('pika_locator'), 'launch', 'three_locator.launch.py')])
    )
    ...
    h_depth_camera_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(get_package_share_directory('realsense2_camera'), 'launch', 'rs_launch.py')]),
        launch_arguments={'serial_no': h_depth_camera_no,
                          'camera_namespace': name,
    ...
    return LaunchDescription(declared_arguments+[
        ...
        Node(
            package='sensor_tools',
            executable='usb_camera.py',
            name=[name_index,TextSubstitution(text='camera_fisheye_h')],
            parameters=[{'camera_port': h_fisheye_port,
                         'camera_fps': camera_fps,
                         'camera_height': camera_height,
                         'camera_width': camera_width,
                         'camera_frame_id': [name,TextSubstitution(text="/camera_fisheye_h_link")]}],
            remappings=[
                ('/camera_rgb/color/image_raw', [name,TextSubstitution(text='/camera_fisheye_h/color/image_raw')]),
                ('/camera_rgb/color/camera_info', [name,TextSubstitution(text='/camera_fisheye_h/color/camera_info')])
            ],
            respawn=True,
            output='screen'
        ),
        ...
        Node(
            package='sensor_tools',
            executable='serial_gripper_imu',
            name=[name_index,TextSubstitution(text='serial_gripper_imu_h')],
            parameters=[{'serial_port': h_serial_port}],
            remappings=[
                ('/imu/data', [name,TextSubstitution(text='/imu_h/data')]),
                ('/gripper/data', [name,TextSubstitution(text='/gripper_h/data')]),
                ('/gripper/ctrl', [name,TextSubstitution(text='/gripper_h/ctrl')]),
                ('/gripper/joint_state', [name,TextSubstitution(text='/gripper_h/joint_state')]),
                ('/gripper/joint_state_ctrl', [name,TextSubstitution(text='/joint_states_h')]),
                ('/joint_state_info', '/joint_states_h'),
                ('/joint_state_gripper', '/joint_states_gripper_h'),
                ('/teleop_trigger', 'teleop_trigger_h'),
                ('/data_capture_status', '/data_tools_dataCapture/status'),
                ('/teleop_status', '/teleop_status_h'),
                ('/localization_status', '/pika_localization_status_h'),
                ('/arm_control_status', '/arm_control_status_h'),
            ],
            respawn=True,
            output='screen'
        )
